...
func (connector *MySQLConnector) createMySQL(config *entity.AppConfig, logicIndex int) *MySQLConnector {
...
func (connector *MySQLConnector) retryOpenDB() *gorm.DB {
	//var err error
	_database, err := gorm.Open("mysql", connector.logic.DBUrl) //keys.RemoteConfig.LogicServer[keys.LogicIndex].DBUrl)
	glog.Error(err)
	...
	_database.DB().SetConnMaxLifetime(time.Second * time.Duration(connector.config.SessionExpires))
	return _database
}
